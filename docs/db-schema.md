...
| `deleted_at` | horodatage | nullable — `NULL` tant que le compte n'est pas soft-supprimé, rempli à la date de suppression logique sinon |
| `token_version` | entier | `NOT NULL`, défaut `0` |
| `created_at` | horodatage | `NOT NULL`, valeur posée par le moteur de base de données via `server_default=now()` |
| `updated_at` | horodatage | `NOT NULL`, posée et rafraîchie via la configuration SQLAlchemy `server_default`/`onupdate=now()` |
...
- `created_at`/`updated_at` : traçabilité, calculées côté base de données via `server_default` plutôt que côté Python, pour rester correctes même en cas d'insertion hors du chemin applicatif habituel (script SQL direct, migration de données). Sur SQLite, `DateTime(timezone=True)` est sans effet : la colonne est un `DATETIME` simple et les valeurs stockées sont en UTC, sans décalage horaire explicite.
